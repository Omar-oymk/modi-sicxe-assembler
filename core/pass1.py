...
def create_tables(line_list):
    ...
    return block_list, pool_list, adjusted_block_list, total_program_length, symb_table


def save_pool_table(pool_list, file_path):
...
def save_block_table(block_list, total_program_length, file_path):
    with open(file_path, 'w') as f:
        f.write(f'{"BLOCK NAME":<12}{"BLOCK NUMBER":<14}{"ADDRESS":<9}{"SIZE":<4}\n')

        for item in block_list:
            values = list(item.values())
            f.write(f'{values[0]:<12}{values[1]:<14}{values[2]:<9}{values[3]:<4}\n')
        
        f.write(f'\nTotal program length: {total_program_length}')

def save_symbol_table(symbol_list, file_path):
    with open(file_path, 'w') as f:
